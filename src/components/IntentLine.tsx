...
export const IntentLine = ({ sensitivity = 70, easeIn = 200, onChordActivated, onActionConfirmed, onLineCreated, onSuggestionDiscarded }: { sensitivity?: number; easeIn?: number; onChordActivated?: () => void; onActionConfirmed?: () => void; onLineCreated?: (distance: number) => void; onSuggestionDiscarded?: () => void }) => {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Handle right arrow to cycle through targets when line is active
      if (isLineActive && (e.code.toLowerCase() === "arrowright" || e.key === "ArrowRight")) {
        e.preventDefault();
        e.stopPropagation();
        cycleTargetButton();
      }
  ...
  const isArrowRightPressed = keysPressed.has("arrowright");
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-50">
      ...
      {isArrowRightPressed && isLineActive && (
        <div 
